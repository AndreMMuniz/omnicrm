...
// ── Daily volume chart ────────────────────────────────────────────────────────
function DailyVolumeChart({ current, previous }: { current: DayPoint[]; previous: DayPoint[] }) {
  ...
  const barGroupW = innerW / current.length;
  const barW = hasPrev ? barGroupW * 0.36 : barGroupW * 0.55;
  ...
  return (
    <div>
      <div className="flex items-center gap-4 flex-wrap mb-1.5">
        ...
        <div className="flex gap-1 p-[3px] bg-slate-100 rounded-lg ml-auto">
          {(["conversations", "messages"] as const).map((opt) => (
            <button key={opt} onClick={() => setView(opt)} style={{
              ...
              background: view === opt ? "white" : "transparent",
              color: view === opt ? C.primary : C.secondary,
              boxShadow: view === opt ? "0 1px 2px rgba(0,0,0,0.06)" : "none",
            }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={220} style={{ display: "block" }}>
        ...
        {current.map((d, i) => {
          ...
          return (
            <g key={d.date} onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(null)}>
              <rect x={groupX} y={padT} width={barGroupW} height={innerH} fill="transparent" />
              {hasPrev && prevD && (
                <rect x={groupX + barGroupW * 0.05} y={padT + innerH - prevBarH}
                  width={barW} height={Math.max(prevBarH, 1)}
                  fill="#e2e8f0" opacity={hover === null || hover === i ? 1 : 0.4} rx={2} />
              )}
              <rect
                x={groupX + (hasPrev ? barGroupW * 0.05 + barW + 3 : barGroupW * 0.22)}
                y={padT + innerH - barH}
                width={barW} height={Math.max(barH, 1)}
                fill={C.primaryContainer} opacity={hover === null || hover === i ? 1 : 0.4} rx={2}
              />
              {(current.length <= 14 || i % 2 === 0) && (
                <text x={groupX + barGroupW / 2} y={H - 8} fontSize={9} fill="#94a3b8" textAnchor="middle" fontFamily="Inter">
                  {formatDate(d.date)}
                </text>
              )}
            </g>
        ...
        {hover !== null && (() => {
          ...
          const x = padL + hover * barGroupW + barGroupW / 2;
          const y = padT + innerH - (d.count / yMax) * innerH - 14;
          const label = prevD ? `${d.count} / ${prevD.count}` : `${d.count}`;
          const labelW = prevD ? 72 : 52;
          return (
            <g>
              <rect x={x - labelW / 2} y={y - 16} width={labelW} height={20} rx={6} fill="#0f172a" />
              <text x={x} y={y - 2} fontSize={11} fill="white" textAnchor="middle" fontFamily="Inter" fontWeight={600}>{label}</text>
            </g>
          );
        })()}
      </svg>
...
function ChannelDonut({ channels }: { channels: Record<string, number> }) {
  ...
  if (!data.length)
    return <div className="text-sm text-slate-400 py-8 text-center">No channel data.</div>;
  ...
  const arcs = data.map((d) => {
    ...
    return { ...d, pct, path: `M ${x1} ${y1} A ${R} ${R} 0 ${large} 1 ${x2} ${y2} L ${x3} ${y3} A ${r} ${r} 0 ${large} 0 ${x4} ${y4} Z` };
  });
...
function AgentTable({ agents }: { agents: AgentStat[] }) {
  if (!agents.length)
    return <div className="text-sm text-slate-400 py-8 text-center">No agent data for this period.</div>;
        ...
        <tbody>
          {agents.map((a, i) => (
            <tr key={a.id} style={{ borderBottom: i === agents.length - 1 ? "none" : `1px solid ${C.outlineVariant}` }}>
              ...
              <td style={{ padding: "10px 8px", color: C.onSurface, fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>
                {a.conversations_handled.toLocaleString()}
              </td>
              <td style={{ padding: "10px 8px", color: C.onSurface, fontVariantNumeric: "tabular-nums" }}>
              ...
              <td style={{ padding: "10px 8px" }}>
                <div className="flex items-center gap-2">
                  ...
                  <span style={{ fontSize: 12, color: C.onSurface, fontVariantNumeric: "tabular-nums" }}>
                    {Math.round(a.resolution_rate)}%
                  </span>
                </div>
              </td>
...
function QueueByChannel({ queue }: { queue: Record<string, number> }) {
  const entries = useMemo(() => {
    return Object.entries(queue)
      .map(([id, value]) => {
        const key = id.toUpperCase();
        const ch = CHANNEL_MAP[key];
        return ch ? { ...ch, value } : { id: key, label: id, color: "#64748B", value };
      })
      .filter((e) => e.value > 0)
      .sort((a, b) => b.value - a.value);
  }, [queue]);
  ...
  if (!entries.length)
    return <div className="text-sm text-slate-400 py-6 text-center">No queued conversations.</div>;
      ...
      {entries.map((e) => (
        <div key={e.id}>
          <div className="flex justify-between mb-1">
            ...
            <span style={{ fontSize: 12, color: C.secondary, fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>
...
// ── AI stats ──────────────────────────────────────────────────────────────────
function AIStats({ stats }: { stats: DashboardStats | null }) {
  const items = [
    { label: "AI suggestions generated", value: (stats?.ai_suggestions_generated ?? 0).toLocaleString(), icon: "auto_awesome",  color: "#7C4DFF", bg: "#f5f3ff" },
    { label: "Conversations with AI",    value: (stats?.convs_with_ai ?? 0).toLocaleString(),            icon: "smart_toy",     color: "#3B82F6", bg: "#eff6ff" },
    { label: "AI adoption rate",         value: `${(stats?.ai_adoption_pct ?? 0).toFixed(1)}%`,          icon: "insights",      color: "#10B981", bg: "#ecfdf5" },
    { label: "Overall resolution rate",  value: stats ? `${(stats.resolution_rate * 100).toFixed(1)}%` : "—", icon: "check_circle", color: "#F59E0B", bg: "#fffbeb" },
  ];
  return (
    <div className="flex flex-col gap-3">
      {items.map((it) => (
        <div key={it.label} className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: C.onSurface, fontVariantNumeric: "tabular-nums", lineHeight: 1.2 }}>
              {it.value}
            </div>
          </div>
        </div>
...
export default function DashboardPage() {
  ...
  const convsSparkline = stats?.daily_conversations?.map((d) => d.count) ?? [];
      ...
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 bg-[#f8fafc] scrollbar-hide">
          ...
          <div className="grid grid-cols-4 gap-4 mb-4">
            ...
            />
            <KPICard
              icon="timer" iconBg="#ecfdf5" iconColor="#10B981"
              value={stats?.avg_first_response_minutes != null
                ? `${Math.round(stats.avg_first_response_minutes)} min`
                : "—"}
              label="Avg first response"
              sub={stats?.avg_resolution_hours != null
                ? `Avg resolution: ${stats.avg_resolution_hours.toFixed(1)}h`
                : undefined}
            />
          </div>

          {/* Row 1: Daily volume + Channel donut */}
          <div className="grid gap-4 mb-4" style={{ gridTemplateColumns: "2fr 1fr" }}>
            <Card
              title="Conversation volume"
              action={<span style={{ fontSize: 11, color: "#94a3b8" }}>Current vs. previous period</span>}
            >
              <DailyVolumeChart
                current={stats?.daily_conversations ?? []}
                previous={stats?.prev_daily_conversations ?? []}
              />
            </Card>
          ...
          {/* Row 2: Agent table + Status funnel */}
          <div className="grid gap-4 mb-4" style={{ gridTemplateColumns: "2fr 1fr" }}>
          ...
          {/* Row 3: Queue by channel + AI stats */}
