    ...
    <main className="flex h-full min-h-0 flex-col bg-[#F8F9FA]">
      <section className="border-b border-[#E9ECEF] bg-white px-6 py-5">
        ...
        <div className="mt-2 flex items-center gap-3">
          <h1 className="text-[24px] font-semibold tracking-[-0.02em] text-slate-900">People</h1>
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-500">
            Phase 1
          </span>
        </div>
        <p className="mt-2 max-w-2xl text-sm text-slate-500">
          Esta rota prepara o novo workspace de contatos no padrão inspirado no Twenty:
          tabela à esquerda e detalhe contextual à direita.
        </p>
      </section>
      <section className="flex flex-1 items-center justify-center px-6 py-10">
        <div className="max-w-xl rounded-[28px] border border-dashed border-slate-300 bg-white px-8 py-10 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-500">
            Na pr&oacute;xima fase, esta tela vai adotar o shell de CRM com filtros, tabela e painel lateral de detalhe.
          </p>
        </div>
      </section>
    </main>
